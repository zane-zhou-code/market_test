...
class time_list(object):
    ...
    def calculated_price(self):
        try:
            ...
            # print(df_new.erdat_new.dt.year)
            print(df_new.erdat_new)
            print(df_new.erdat_new.apply(self.special_fuc))
...
def main():
    ...
    sel_sql2 = '''select dim_id, erdat, close_price
                      ...
                      where dim_id='617100' --617100 615600 622600
                      and erdat between '20200301'
                      and '20200401'
                      order by erdat'''
    ins_sql = '''insert into DIM_MARKET_PRICE_FORMULA (dim_id_dr, erdat, calced_price, dim_id)values('%s', '%s', %s, '%s')'''
    result1 = sql().sql_req(conn, sql=sel_sql1)
    all_data = sql().sql_req(conn,  sql=sel_sql2)
    all_results = time_list_new(all_data).result_cal()
    print(all_results)
    # for i in range(0, len(result1)):
    #     all_data = sql().sql_req(conn, (result1[i][0], yesterday, yesterday), sql=sel_sql2)
    #     if all_data:
    #         all_results = time_list_new(all_data).result_cal()
    #         for i in range(0, len(all_results)):
    #             if all_results[i][1] >= day_weekbefore:
