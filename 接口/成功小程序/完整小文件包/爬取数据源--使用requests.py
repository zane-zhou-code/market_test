...
# 定义hana连接
conn = get_connection().hana_connection('192.168.2.191', '30041', 'XHZHOU', 'Zhou1234')
# 定义两个全局变量
...
def parse_one_page(html):
    pattern = re.compile('<td align="center">(.*?)</td>.*?<td align="center">(.*?)</td>.*?<td align="center">(.*?)</td>', re.S)
    items = re.findall(pattern, html)
    for item in items:
        if item ==[]:
            print('\033[1;37;41m----------------------CCF价格逻辑异常-----------------------\033[0m')
        else:
            yield {
                'index': item[0],
...
def parse_other_page(html, header, cookies):
    pattern = re.compile('<li(?: | class=articlebreak )><span>.*?href="(.*?)" class="h1a2" target="_blank" onmouseover="return overlib.*?</a></li>', re.S)
    items = re.findall(pattern, html)
    # print(items)
